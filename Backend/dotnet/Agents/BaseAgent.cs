...
public interface IAgent
{
    ...
    /// <summary>
    /// Respond with additional dynamic tools (e.g., MCP tools wrapped as AIFunctions).
    /// The LLM will decide when to call these tools.
    /// </summary>
    Task<string> RespondWithToolsAsync(string message, IEnumerable<AIFunction>? additionalTools = null, List<GroupChatMessage>? conversationHistory = null, string? context = null);
...
public abstract class BaseAgent(ILogger logger, ActivitySource? activitySource = null) : IAgent
{
    ...
    {
        // Default implementation without additional tools
        return await RespondWithToolsAsync(message, null, conversationHistory, context);
    }

    /// <summary>
    /// Respond with optional additional tools. Base implementation uses chat client without tools.
    /// Override in derived classes to support function calling with tools.
    /// </summary>
    public virtual async Task<string> RespondWithToolsAsync(string message, IEnumerable<AIFunction>? additionalTools = null, List<GroupChatMessage>? conversationHistory = null, string? context = null)
    {
